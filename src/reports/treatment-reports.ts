...
export async function generateTreatmentReportBuffer(
  fromDate: Date,
  toDate: Date,
  t: TFunction,
  locale: string = "de",
  ...
  const treatments = await appDrizzle.query.treatments.findMany({
    where: { AND: [{ startDate: { gte: fromDate } }, { startDate: { lte: toDate } }] },
    with: { drug: true, animalTreatments: { with: { animal: { with: { earTag: true } } } } },
  ...
  for (const treatment of treatments) {
    for (const at of treatment.animalTreatments) {
      ...
      if (animalTypes && animalTypes.length > 0 && !animalTypes.includes(animal.type)) continue;

      if (!rowsByType.has(animal.type)) rowsByType.set(animal.type, []);
      ...
      if (treatment.drugDoseValue != null && treatment.drugDoseUnit) {
        ...
        if (treatment.drugDosePerUnit) dose += ` / ${t(`drug_dose_per_units.${treatment.drugDosePerUnit}`)}`;
      }
      ...
      rowsByType
        ...
        .push([
          ...
          treatment.drug?.name ?? "",
          dose,
          treatment.isAntibiotic ? "✓" : "",
          treatment.criticalAntibiotic ? "✓" : "",
          treatment.antibiogramAvailable ? "✓" : "",
          milkDays,
          meatDays,
          organsDays,
  ...
  for (const type of typesToProcess) {
    ...
    const sheet = workbook.addWorksheet(t(`animal_types.${type}`));
    let rowIndex = 1;
