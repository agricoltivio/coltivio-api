...
export async function generateOutdoorJournalReportBuffer(
  fromDate: Date,
  toDate: Date,
  t: TFunction,
  locale: string = "de"
): Promise<{ buffer: Buffer; fileName: string }> {
  const herds = await appDrizzle.query.herds.findMany({
    with: { herdMemberships: { with: { animal: true } }, outdoorSchedules: { with: { recurrence: true } } },
  });

  type ExpandedOccurrence = { herdId: string; type: "pasture" | "exercise_yard"; startDate: Date; endDate: Date };
      ...
      for (const occ of allOccurrences) {
        ...
        for (const membership of herd.herdMemberships) {
          ...
          if (!monthData.has(category)) {
            monthData.set(
              category,
              Array.from({ length: days }, () => ({ pasture: false, exerciseYard: false }))
            );
          }
  ...
  for (let col = 2; col <= 1 + totalDataCols; col++) sheet.getColumn(col).width = 8;
  ...
  for (let day = 1; day <= 31; day++) {
    ...
    for (let m = 0; m < numMonths; m++) {
      ...
      for (let c = 0; c < numCats; c++) {
        const dataCell = sheet.getCell(rowIndex, colFor(m, c));
        dataCell.alignment = { horizontal: "center" };
        dataCell.border = borderFor(c);
  ...
  for (let m = 0; m < numMonths; m++) {
    for (let c = 0; c < numCats; c++) {
      const dataCell = sheet.getCell(rowIndex, colFor(m, c));
      dataCell.value = pastureCount[m][c] || "";
      dataCell.alignment = { horizontal: "center" };
      dataCell.font = { bold: true };
      dataCell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFD9E2F3" } };
      dataCell.border = borderFor(c);
    }
  }
  rowIndex++;
  ...
  for (let m = 0; m < numMonths; m++) {
    for (let c = 0; c < numCats; c++) {
      const dataCell = sheet.getCell(rowIndex, colFor(m, c));
      dataCell.value = exerciseYardCount[m][c] || "";
      dataCell.alignment = { horizontal: "center" };
      dataCell.font = { bold: true };
      dataCell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFD9E2F3" } };
      dataCell.border = borderFor(c);
