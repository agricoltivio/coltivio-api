...
import { listPermissionsForUser, setFeatureAccess, resetFeatureAccess } from "./farm-permissions";
...
export const listMemberPermissionsEndpoint = farmEndpointFactory.build({
  ...
  handler: async ({ input }) => {
    const result = await listPermissionsForUser(input.userId);
    return { result };
  },
});
...
export const setMemberPermissionEndpoint = ownerOnlyEndpointFactory.build({
  ...
  handler: async ({ input, ctx }) => {
    await setFeatureAccess(input.userId, ctx.farmId, input.feature, input.access);
    return {};
  },
});
...
export const resetMemberPermissionEndpoint = ownerOnlyEndpointFactory.build({
  ...
  output: z.object({}),
  handler: async ({ input }) => {
    await resetFeatureAccess(input.userId, input.feature);
    return {};
  },
});
